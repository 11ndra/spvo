...
| C6-01 | Гл.6 | Правило Suricata состоит из action, header и rule options; эта структура не обобщается на все IDPS | OFFICIAL/QUALIFIED | VERIFIED | Suricata Rule Guide; в тексте явно ограничено конкретным движком |
| C6-02 | Гл.6 | `http.uri` выбирает нормализованное представление HTTP URI, после чего `content` применяется к выбранному sticky buffer | OFFICIAL | VERIFIED | Suricata 8.0.7 HTTP Keywords |
| C6-03 | Гл.6 | `flow:established,to_server` задаёт контекст состояния/направления, но само ключевое слово не является отдельной методологией обнаружения | OFFICIAL/COURSE SYNTHESIS | VERIFIED / QUALIFIED | Suricata 8.0.7 Flow Keywords + разграничение Главы 5 |
| C6-04 | Гл.6 | `msg`, `sid`, `rev` идентифицируют/описывают правило и его результат, но не заменяют условие совпадения | OFFICIAL | VERIFIED | Suricata Meta Keywords |
| C6-05 | Гл.6 | `drop` имеет предотвращающий эффект только в подходящем IPS/inline-режиме | OFFICIAL | VERIFIED | Suricata configuration/action-order documentation |
| C6-06 | Гл.6 | Thresholding управляет частотой оповещений и не исправляет логическую причину слишком широкого условия; `drop`/`reject` в IPS имеют отдельную семантику | OFFICIAL/COURSE SYNTHESIS | VERIFIED / QUALIFIED | Suricata Thresholding Keywords; оговорка сохранена в тексте |
| C6-07 | Гл.6 | Учебное правило `SID 1000001` и маркер `ATTACK-LAB` являются синтетическим примером, а не реальным IOC | SYNTHETIC | VERIFIED AS COURSE MATERIAL | Используется тот же контролируемый артефакт, что и в ЛР №1 |
| L6-01 | Гл.6 (legacy) | Base-rate способен приводить к большому числу FP даже при низком FPR | RESEARCH | VERIFIED | Stefan Axelsson, ACM TISSEC; добавлена прямая research reference |
| L6-02 | Гл.6 (legacy) | Любое «ужесточение правила» обязательно повышает FN | ENGINEERING | NEEDS QUALIFICATION | Исправлено: trade-off показан корректно для движения decision threshold; произвольная правка rule не обязана быть монотонной |
| L6-03 | Гл.6 (legacy) | Log4Shell scan-source IP давали высокий FP; CISA советовала искать successful exploitation | CASE | VERIFIED | CISA AA21-356A, прямое утверждение advisory |
| L6-04 | Гл.6 (legacy) | Разделять recon/attempt/success и не блокировать по low-confidence indicator | ENGINEERING | VERIFIED AS INFERENCE | Теперь явно названо инженерным расширением CISA guidance, не «best practice» без источника |
