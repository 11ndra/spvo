!!! warning "Материал предыдущей редакции"
    Эта страница сохранена только как рабочий материал миграции и **не входит в текущую последовательность занятий**. Актуальная структура курса сейчас доведена до Главы 4 и ЛР №3. Содержимое этой страницы будет переработано перед использованием со студентами.
...
!!! note "Методика курса"
    Эта цепочка — **наша инженерно-педагогическая модель**, а не обязательный workflow, опубликованный OISF как стандарт. Её идея согласуется с практикой воспроизводимого тестирования: OISF `suricata-verify` запускает Suricata с заданными входами/правилами и проверяет ожидаемый output. В курсе этот принцип расширен до явных positive, negative и variant cases.

---

## 2. Сначала поведение, затем observable

Допустим, учебная цель сформулирована так:

> **Обнаруживать traversal-подобный ввод в параметре `file` учебного endpoint `/download`.**

Это **не равно** цели «доказать успешный выход приложения за Document Root». Последняя зависит от серверной конфигурации, нормализации, маршрутизации приложения и файловой системы и обычно не может быть установлена только по URI сетевого запроса. Для неё потребовался бы независимый server-side ground truth.
...
> В **HTTP URI запроса к серверу на `/download`** параметр `file` содержит последовательность `../`. Это наблюдаемый признак учебного use case, а не доказательство успешного traversal на сервере.
...
!!! note "Проверено по Suricata 8"
    На дату обновления 16.09.2026 актуальный release ветки Suricata 8 — **8.0.7**. Официальная документация Suricata 8 описывает `http.uri` как нормализованный URI buffer, а `http.uri.raw` — как raw URI buffer.
...
Первый относится к определённой выше учебной цели `/download?file=...`.

Второй содержит тот же маркер, но находится вне целевого endpoint и поэтому служит отрицательным примером **относительно этой конкретной цели обнаружения**.
...
Вариант должен изменять **наблюдаемое представление**, а не только имя целевого файла. Например, можно проверить percent-encoded форму:

```text
/download?file=%2e%2e%2f%2e%2e%2fetc%2fpasswd
```

Такой тест не имеет заранее гарантированного результата: нужно отдельно установить, как выбранный sensor строит `raw` и нормализованное URI-представление и как тот же ввод интерпретирует учебное приложение.

Отдельный класс тестов — **transport robustness**: тот же маркер передаётся через несколько TCP-сегментов и проверяет reassembly, а не URI normalization.
...
| `/download?file=%2e%2e%2f%2e%2e%2fetc%2fpasswd` | вариант представления | результат устанавливается экспериментом для raw/normalized URI |

Теперь правило можно оценивать не по одному событию, а по **матрице ожидаемого поведения**.

Это первая форма regression testing для **заданного набора случаев**. Она подтверждает ожидаемое поведение только на проверенной выборке и не является доказательством общей надёжности детектора во всех возможных представлениях и условиях эксплуатации.
...
Но нужно различать логическую причину совпадения и механизм ограничения частоты событий. В Suricata thresholding управляет тем, **как часто создаются alerts** для соответствующей сигнатуры; для некоторых prevention actions действие над пакетами может продолжаться даже при ограниченной частоте alert.

Если сигнатура логически совпадает с нормальным трафиком, thresholding:

> не превращает такое совпадение в истинное обнаружение.

Он может уменьшить alert volume, но не исправляет само условие detection.
