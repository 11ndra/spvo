...
for cmd in ip curl jq; do command -v "$cmd" >/dev/null 2>&1 && ok "команда: $cmd" || bad "не найдена команда: $cmd"; done
ip -o -4 addr show | grep -q '10\.13\.37\.10/' && ok 'адрес 10.13.37.10 найден' || bad 'нет 10.13.37.10/24'
