...
6. Сгенерировать соседний контролируемый поток, который по топологии не должен проходить через выбранную точку.
7. Независимо подтвердить, что этот соседний поток действительно был создан и дошёл до своего назначения.
8. Проверить счётчики/захват на сенсоре и отсутствие этого потока в наблюдаемой выборке.
```

Отсутствие записи у сенсора **само по себе не доказывает** границу видимости: причиной могут быть потеря пакетов, фильтр захвата, ошибка маршрута или конфигурация. Граница подтверждается только контролируемым экспериментом с независимым доказательством существования потока и состояния capture path.

Так проверяется не только «что система видит?», но и **какие утверждения о границах видимости действительно поддерживаются экспериментом**.
