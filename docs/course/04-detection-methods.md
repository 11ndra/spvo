...
<div class="quiz" data-question-id="chapter4-v224-q3">
  <p><strong>Запрос подтверждён журналом web-сервера, но на одном интерфейсе tcpdump его не показывает. Какой вывод допустим?</strong></p>
...
Для HTTP-теста журнал учебного web-сервиса используется как независимое подтверждение прикладной доставки. Одновременно в лабораторной отдельно оговорено, что application log не заменяет наблюдение L3/L4 и не может служить универсальным ground truth для любого сетевого события.
