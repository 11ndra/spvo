...
Рассмотрим сенсор на пути HTTPS-соединения.
...
Актуальная спецификация TLS 1.3, RFC 9846 (она заменяет RFC 8446), устанавливает, что handshake-сообщения после `ServerHello` защищаются шифрованием. Это означает не «сетевой IDS больше ничего не видит», а более точное утверждение:
...
AV/EDR может быть **дополнительным хостовым источником и защитным механизмом**, но его нельзя автоматически объявлять «HIDS» только по названию продукта.
...
- RFC 9846 — актуальная спецификация TLS 1.3, заменившая RFC 8446; используется для объяснения того, какие части handshake защищаются и почему содержимое приложения недоступно внешнему сетевому сенсору без дополнительной архитектуры.
