...
<div class="idps-axis-table-wrap">
  <table class="idps-axis-table">
    ...
    <tbody>
      ...
      <tr><th>Результат обнаружения</th><td>Сформированный движком alert/event или иной результат выполненной логики</td><td>Результат — это семантический итог детектора, а не имя файла или канала вывода.</td></tr>
      <tr><th>Вывод / хранение</th><td>EVE JSON и другие настроенные выходы</td><td><code>eve.json</code> сериализует и хранит настроенные записи; сам файл не выполняет обнаружение.</td></tr>
    </tbody>
  </table>
</div>
