...
!!! info "Статус проверки лабораторной"
    - `DESIGN VERIFIED` — эксперимент меняет точку наблюдения относительно двух контролируемых сетевых путей и использует независимый `tcpdump` для подтверждения наличия пакетов.
    - `STATIC QA PASSED` — проверены пути, актуальный bundle `v0.2`, shell syntax, команды Suricata/tcpdump и обработка offloading/checksum assumptions.
    - `RUNTIME QA REQUIRED` — полный двухинтерфейсный end-to-end прогон в VirtualBox на эталонных Ubuntu 24.04.x ещё не выполнен.
...
!!! note "О статусе воспроизводимости"
    ЛР №3 v0.2 прошла статическую проверку скриптов и компонентов. Полный эксперимент VirtualBox → Ubuntu 24.04.x → два интерфейса → tcpdump → Suricata по-прежнему требует контрольного end-to-end прогона на эталонных VM перед присвоением статуса `RUNTIME VERIFIED`.
