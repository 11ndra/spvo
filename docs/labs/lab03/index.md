...
!!! note
    Мы намеренно не меняем `suricata.yaml` глобально. Это локальная мера для воспроизводимого учебного эксперимента, а не рекомендация для рабочей среды.
...
Обратите внимание: учебный веб-сервис слушает **только** `10.13.37.20:8080`, а не `0.0.0.0:8080`. Поэтому подтверждение не смешивает запросы к учебному и NAT-адресу сервера.
...
3. зачем в учебном запуске используется `-k none` и почему это не рекомендация для рабочей среды.
...
3. Слушает ли веб-сервис именно 10.13.37.20:8080?
