...
[Скачать пакет ЛР №2 v0.3](../../assets/downloads/idps-lab02-bundle-v0.3.zip){ .md-button }
...
```bash
cd ~
unzip idps-lab02-bundle-v0.3.zip
cd idps-lab02-bundle-v0.3
...
```mermaid
flowchart LR
    C[idps-client\n10.13.37.10] -->|HTTP GET /lab2-trigger/LAB2-NET| S[idps-server\n10.13.37.20:8080]
...
sudo suricata \
  -k none \
  -c /etc/suricata/suricata.yaml \
  -S "$HOME/idps-lab02-bundle-v0.3/server/lab02.rules" \
  -i "$LAB_IFACE" \
  -l "$HOME/lab02-output"
```
...
## 10. Что сдаётся и как защищается

Используйте `report/lab02-report.md`.

На защите преподаватель выбирает один источник, а студент должен:

1. показать исходное подтверждение, а не пересказ;
2. назвать минимум два факта, которые оно действительно подтверждает;
3. назвать минимум один факт, который из него не следует;
4. объяснить, почему второй источник дополняет первый.
