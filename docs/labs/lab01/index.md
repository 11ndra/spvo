...
!!! info "Статус проверки лабораторной"
    - `DESIGN VERIFIED` — причинно-следственная логика эксперимента проверена: существование запроса подтверждается независимо от оповещения, а alert не трактуется как доказательство компрометации.
    - `STATIC QA PASSED` — проверены пути, актуальный bundle `v0.4`, shell syntax и обязательные зависимости инструкции.
    - `RUNTIME QA REQUIRED` — полный end-to-end сценарий на эталонных Ubuntu Desktop/Server 24.04.x ещё не получил статус `RUNTIME VERIFIED`.
