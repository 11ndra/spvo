...
<p>развернуть минимальный сетевой стенд; определить observation interface; доказать наличие трафика до запуска IDS; установить и проверить Suricata; создать простую HTTP-сигнатуру; получить контролируемый alert; найти его в EVE JSON; объяснить разницу между совпадением правила и подтверждённой компрометацией; показать первый пример False Positive.</p>
</div>
...
Для Ubuntu официальный Suricata Quickstart рекомендует stable PPA OISF для установки актуальной стабильной версии.
...
| Suricata установлена, configuration test пройден | 15 |
