...
В лабораторных системный экземпляр Suricata не должен запускаться автоматически: студент будет запускать отдельный процесс вручную на конкретном учебном интерфейсе. `bootstrap-server.sh` версии 1.2 сам выполняет `systemctl disable --now suricata`. После bootstrap проверьте итоговое состояние:
...
Проверяются не только IP-адреса и наличие инструментов. Версия 1.2 также подтверждает, что:

- NAT и Host-Only используют `virtio_net`;
- существует ровно один маршрут `default`, и он не идёт через LAB-интерфейс;
- маршрут между `10.13.37.10` и `10.13.37.20` проходит через LAB-интерфейс;
- hostname сравнивается без учёта регистра;
- на сервере `auditd` активен и Linux Audit находится в состоянии `enabled 1`;
- правило `never,task` не мешает сбору ожидаемой audit-телеметрии;
- системный сервис Suricata находится в состояниях `inactive` и `disabled`;
- SSH service активен и TCP/22 находится в состоянии LISTEN;
- Suricata относится к поддерживаемой ветке 8.x и проходит базовый `-T`.
