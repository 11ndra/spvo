...
flowchart LR
    ...
    J --> ST[машина состояний]
    J --> BH[фиксированный временной порог]
    J --> AN[сравнение с базовый профиль]
```
...
Поле `phase` нужно только для разделения базовый профиль и всплеск в аномальном тесте. Оно не является признаком атаки.
...
Пауза перед всплеск нужна только для чистоты эксперимента: она гарантирует, что двухсекундное окно поведенческого детектора не содержит хвост базовый профиль. Внутри всплеск задержка не добавляется.
...
Детектор не анализирует частоту, базовый профиль или состояние session-протокола. Он отвечает на один вопрос:
...
## 7. Примените машина состояний detector
...
<div class="lab-evidence">
<strong>Допустимый вывод</strong>
<p>Наблюдаемая последовательность нарушила заданную машина состояний учебного протокола.</p>
</div>
...
## 9. Сравните всплеск с базовый профиль
...
Детектор берёт события с меткой `базовый профиль`, вычисляет медианный интервал между ними и сравнивает его с всплеск.
...
Точные значения интервалов зависят от скорости VM. Важен принцип: всплеск должен быть как минимум в три раза быстрее базовый профиль по медианному интервалу.
...
<div class="lab-evidence">
<strong>Допустимый вывод</strong>
<p>В данном контролируемом прогоне временная структура всплеск существенно отклонилась от измеренного базовый профиль по выбранному критерию.</p>
</div>
...
| Метод | Основание решения | Нужна история нескольких событий? | Нужен базовый профиль? |
|---|---|---:|---:|
...
<p>Оба детектора анализировали частоту `/catalog`, но поведенческий применял фиксированный порог, а аномальный сравнивал всплеск с измеренным базовый профиль.</p>
</div>
...
4. Почему фиксированный порог `5 за 2 секунды` является поведенческим условием, но не требует anomaly базовый профиль?
...
| `MATCHES=0` для behavior | Выполнялся ли всплеск без искусственной задержки |
| `ANOMALY=NO` | Сравните интервалы базовый профиль/всплеск; повторите чистый эксперимент на ненагруженных VM |
