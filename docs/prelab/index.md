...
!!! warning "Статус материала"
    Эта страница относится к предыдущей структуре курса v2.16.1 и сохранена для совместимости. Главы 1–6 и ЛР №1–5 уже переведены на новую дидактическую структуру; эта предварительная проверка сохранена только как материал предыдущей редакции и будет переработана позже.
...
<div class="quiz assessment-question" data-question-id="pre-3" data-domain="architecture">
  ...
  <p><strong>Что это показывает?</strong></p>
    <button data-correct="true">A. Access policy и IPS — разные логические функции, даже если они реализованы внутри одного NGFW</button>
...
<div class="next-step">
<strong>После зачёта:</strong> переходите к <a href="../labs/lab01/">ЛР №1 — NIDS и первый detection</a>. В лабораторной нужно будет уже руками доказать цепочку: <em>трафик дошёл до сенсора → правило было применено → событие появилось → результат можно объяснить</em>.
</div>
