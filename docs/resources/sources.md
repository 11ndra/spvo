...
## Глава 11 — сопоставление нескольких источников и контекст угроз

Глава продолжает доказательную модель курса: корреляция рассматривается как формализованное условие над несколькими наблюдениями, а не как способ автоматически получить «единую истину». Каждый источник сохраняет собственное происхождение, представление и границу доказательности.

Основные источники:

- OASIS, *STIX Version 2.1* — стандарт машинно-читаемого представления информации об угрозах: https://docs.oasis-open.org/cti/stix/v2.1/stix-v2.1.html
...
- NIST SP 800-150, *Guide to Cyber Threat Information Sharing* (2016) — фундаментальная общая модель информации о киберугрозах, индикаторов, источников и обмена; современные детали реализации проверяются по более новым первичным источникам: https://csrc.nist.gov/pubs/sp/800/150/final
- NIST SP 800-61 Rev. 3 — актуальные рекомендации 2025 года по реагированию; используется только для связи информации об угрозах и совместного анализа с последующими процессами реагирования, а не как структура главы: https://csrc.nist.gov/pubs/sp/800/61/r3/final
- MITRE ATT&CK, *Get Started / FAQ* — ATT&CK как база знаний о поведении противника, а не источник локальной телеметрии: https://attack.mitre.org/resources/ и https://attack.mitre.org/resources/faq/
- MITRE ATT&CK, *ATT&CK Data & Tools* — официальное представление ATT&CK в STIX и доступ через TAXII: https://attack.mitre.org/resources/working-with-attack/
- Suricata 8.0.7, *EVE JSON Format* — `flow_id`, `tx_id` и сопоставление записей EVE внутри модели Suricata: https://docs.suricata.io/en/suricata-8.0.7/output/eve/eve-json-format.html
- Suricata 8.0.7, *suricata.yaml* — Community ID как дополнительный механизм сопоставления сетевых потоков между совместимыми средствами: https://docs.suricata.io/en/suricata-8.0.7/configuration/suricata-yaml.html
- Zeek 9.0.0 LTS, `conn.log` и руководство по журналам — `uid` как внутренний идентификатор соединения и необязательное поле `community_id`: https://docs.zeek.org/en/lts/scripts/base/protocols/conn/main.zeek.html и https://docs.zeek.org/en/lts/tutorial/logs.html

Дополнительная книга Joshua Wright, *Dynamic Incident Response: A Framework for Security Teams* (SANS Institute, 2026) используется для эксплуатационных примеров сочетания источников, применения информации об угрозах и расширения области проверки. Она не задаёт универсальную классификацию корреляции и не заменяет первичные стандарты/официальную документацию.

Ключевые границы главы:

- агрегация записей не равна доказанной связи между ними;
- временная близость не доказывает причинность;
- `flow_id` Suricata и `uid` Zeek являются внутренними идентификаторами своих реализаций;
- Community ID помогает связывать совместимые сетевые представления, но не является универсальным идентификатором события после NAT, прокси или других преобразований;
- совпадение индикатора добавляет внешний контекст, но не доказывает компрометацию или атрибуцию;
- STIX описывает структуру информации, TAXII — механизм обмена;
- ATT&CK является базой знаний о поведении противника, а не первичным источником событий.
