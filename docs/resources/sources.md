...
!!! info "Политика источников"
    Курс использует принцип **primary sources first**. Технический факт по возможности опирается на стандарт, официальную документацию проекта, первичный или близкий к первичному incident report либо исследовательскую публикацию. Архитектурные выводы обозначаются как инженерные рекомендации, а созданные для курса топологии, PCAP и сценарии — как synthetic teaching material.
...
- CISA / Microsoft incident reports — как case sources, если конкретный эпизод демонстрирует разбираемый механизм;
...
По состоянию на 19.09.2026 актуальный stable release ветки Suricata 8 — **8.0.7 (15.09.2026)**; ветка Suricata 7 объявлена EOL. В лаборатории студент всё равно фиксирует фактически установленную версию через `suricata --build-info`, поскольку учебные среды могут обновляться.
...
Основной технический reference Главы 6 — официальная документация Suricata 8.x. Глава использует Suricata как конкретную реализацию rule-based detection и не обобщает её синтаксис на все IDS/IPS.
...
- RFC 9846, TLS 1.3 — актуальная спецификация, заменившая RFC 8446; handshake после `ServerHello` защищается шифрованием, что используется для объяснения границы видимости сетевого сенсора: https://www.rfc-editor.org/rfc/rfc9846.html
...
- NIST SP 800-94 — исторический фундаментальный источник по оценке, false positive/false negative, tuning и эксплуатационным факторам IDPS. Документ 2007 года используется как базовый инженерный reference, а не как описание современной продуктовой taxonomy: https://csrc.nist.gov/pubs/sp/800/94/final
- Stefan Axelsson — *The Base-Rate Fallacy and the Difficulty of Intrusion Detection*; источник для объяснения влияния низкой базовой частоты на практический precision intrusion detection: https://doi.org/10.1145/357830.357849
- Suricata 8.0.7 EVE Index — конкретные counters для проверки capture/resource/output-состояния реализации (`kernel_drops`, exception-policy counters, `alert_queue_overflow`, `alerts_suppressed` и др.): https://docs.suricata.io/en/suricata-8.0.7/appendix/eve-index.html

Глава различает:

- оценку конкретного правила;
- оценку детектора/движка;
- оценку защитного контроля end-to-end;
