...
`START → DATA → END` не является стандартом HTTP, а поле `phase` — служебная метка контролируемого эксперимента, не индикатор атаки. Shell/Python syntax и логика analyser проверены статически; полный двухмашинный сценарий имеет статус `RUNTIME QA REQUIRED`.
