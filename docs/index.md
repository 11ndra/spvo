<div class="home-hero">
  ...
  <div class="home-hero-question">
    ...
    <strong>Лекция → встроенная самопроверка → воспроизводимый лабораторный эксперимент → защита evidence.</strong>
  </div>
...
<div class="big-question-grid">
  ...
  <article class="big-question">
    ...
    <p>Базовые идеи закрепляются интерактивной самопроверкой и лабораторией: сначала наблюдение и evidence, затем вывод.</p>
  </article>
</div>
...
Четыре опорных отношения главы вынесены в отдельные визуальные блоки, а сетевые схемы показывают физический путь traffic и отдельную точку наблюдения NIDS.
...
alert in EVE JSON
      ↓
student defends the interpretation
```

Студент сдаёт собственные evidence и на защите объясняет, что именно подтверждает его эксперимент и какие выводы из alert делать нельзя.
