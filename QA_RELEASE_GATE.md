...
- при обновлении поверх существующей Git working tree — отсутствие stale-файлов, которые новый ZIP больше не содержит (overlay extraction сама их не удаляет); при необходимости использовать `python tools/migrate_v228.py --apply`.
...
- наличие `.gitattributes` с каноническим `LF` для текстовых файлов;
- отсутствие `CRLF`/lone `CR` в контролируемых текстовых файлах, особенно `*.sh`;
